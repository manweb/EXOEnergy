...
{
  std::cout << Form("Evaluating shape agreement 1D for observable %s and separation %s ...",var.GetName(),sep.Data()) << std::endl;
  sep.ToLower();
...
{
  std::cout << Form("Evaluating SS-fraction agreement for %s , %s and %s ... ", nameSS.Data(), nameMS.Data(), name.Data()) << std::endl;
...
{
  std::cout << Form("Filling source data and MC 1D histograms for observable %s and separation %s ...",var.GetName(),sep.Data()) << std::endl;
  ...
  for(size_t iRun = 0; iRun < nRuns; iRun++)
  {
    ...
    if(dataFile.IsZombie())
      continue;
    TEntryList* dataSelection = (sep=="all") ? dynamic_cast<TEntryList*>(dataFile.Get("EventList")) : dynamic_cast<TEntryList*>(dataFile.Get(Form("EventList_%s",sep.Data())));
    if(not dataSelection)
      continue;
    std::cout << "With tree name: " << dataSelection->GetTreeName() << " and preprocced file: " << dataSelection->GetFileName() << std::endl;
    ...
    // mc histo
    TString mcName = fUseWeeklyResol ? Form("%s_%s",mcFiles[iRun].c_str(),weeks[iRun].c_str()) : mcFiles[iRun].c_str();
    if(processedMCs.count(mcName) <= 0)
    {
      ...
      TEntryList* mcSelection = (sep=="all") ? dynamic_cast<TEntryList*>(mcFile->Get("EventList")) : dynamic_cast<TEntryList*>(mcFile->Get(Form("EventList_%s",sep.Data())));
      if(not mcSelection)
        continue;
      ...
      if(varName.Contains("energy"))
      {
        ...
        fResolution->FillSmearedMCHisto1D(*hMC, "Rotated", mcChain->GetV1(), mcChain->GetV2(), mcChain->GetSelectedRows(), multiplicity, atoi(startTimes[iRun].c_str()), 0, 1);
      }
      else
      {
        std::cout << var.GetName() << std::endl;
        ...
        FillSmearedMCHisto1D(*hMC, mcChain->GetV1(), GetEnergyObservableIn(GetObservablesIn(fObsList,sep,"data")), "Rotated", mcChain->GetV2(), mcChain->GetV3(), mcChain->GetSelectedRows(), multiplicity, atoi(startTimes[iRun].c_str()), 0, 1);
      ...
      std::cout << Form("Done working on MC file : %s , for week : %s , w/ cut : %s ...",mcFiles[iRun].c_str(),weeks[iRun].c_str(),cutMC.Data()) << std::endl;
    }
...
{
  std::cout << Form("Filling LB data and MC 1D histograms for observable %s and separation %s ...",var.GetName(),sep.Data()) << std::endl;
  ...
  for(int i = 0; i < fObsList.getSize(); i++)
  {
    ...
    if(obs)
    {
      ...
      {
        bool addObs = true;
        for(std::set<TString>::iterator sepIter = fSeparation.begin(); sepIter != fSeparation.end(); sepIter++)
        {
  ...
  SetResultWspPars(result,wsp,false);
  pdf->fillHistogram(mcHisto,RooArgList(var),1.0,&projVars,kTRUE,0,kFALSE);
  //pdf->fillHistogram(mcHisto,RooArgList(var),1.0,&projVars);
  //mcHisto->Sumw2();
  mcHisto->Scale(pdf->expectedEvents(RooArgList(var))/mcHisto->Integral());
  mcHisto->Sumw2();
  data->fillHistogram(dataHisto,RooArgList(var));
  dataHisto->Sumw2();
  //std::cout << pdf->expectedEvents(0) << " " << pdf->expectedEvents(RooArgList(var)) <<  " mc histo: " << mcHisto->Integral() << " data: " << dataHisto->Integral() << std::endl;
  dataHisto->Add(mcHisto,-1);

  // make mc with pdf only
  mcHisto->Reset("ICES");
  SetResultWspPars(result,wsp,true);
  //(pdf->getVariables()->find("beta_scale"))->setVal(1.0);
  //pdf->getVariables()->find("beta_scale")->Print();
  //pdf->getVariables()->Print();
  std::cout << pdf->getNorm() << std::endl;
  //pdf->find("bb2n_Pdf_ss_cached")->Print();
  if (name.Contains("ss"))
  {
  ...
  //mcHisto->Scale(1.0);
  mcHisto->Sumw2();
  //pdf->fillHistogram(mcHisto,RooArgList(var),pdf->expectedEvents(RooArgList(var)),&projVars);
  //pdf->fillHistogram(mcHisto,RooArgList(var),1.0,&projVars);
...
bool EXOSourceAgreement1D::FillSmearedMCHisto1D(TH1D& histo, double* vars, RooRealVar* energy, const std::string& channel, double* energies, double* weights, int length, int multiplicity, long int seconds, int nano, int binMC)
{
  if(not energy)
    return false;
  ...
  for(int b = 1; b <= histo.GetNbinsX(); b++)
  {
    hAux.Reset();
    if(not vs[b-1].empty())
      fResolution->FillSmearedMCHisto1D(hAux,channel,&vs[b-1][0],&ws[b-1][0],vs[b-1].size(),multiplicity,seconds,nano,binMC);
