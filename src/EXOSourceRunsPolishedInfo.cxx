...
void EXOSourceRunsPolishedInfo::SetMCSelectionFileNames(const char* cutMCname, const char* cutMCdir, const char* multWildcard)
{
  ...
  for(std::map<Int_t, std::vector<EXOMetadata> >::iterator runMD = fAllMD.begin(); runMD != fAllMD.end(); runMD++)
  {
    ...
    if(FindMetadata("MCTreeFileName",vecMD))
    {
      ...
      if(strCutMCname != "")
        mcFileName.ReplaceAll(".root",strCutMCname.Data()); // based on Mike's code that worked with .disc.root
        //mcFileName.ReplaceAll(".cut.root",strCutMCname.Data()); // manuel updated to include .cut.root as the file ending... in r11185 ... and having .cut.cut breaks things.
...
const std::string EXOSourceRunsPolishedInfo::GetMCTreeFileName(TString sourceName, int positionS, double positionX, double positionY, double positionZ)
{
  ...
  //fileName += ".cut.root";
  fileName += ".cut.root"; // based on Mike's code that worked with .disc.root
...
void EXOSourceRunsPolishedInfo::SetWiresNoiseLevelPeriods()
{
  ...
  // low noise are indices: 2,4 and 5
  // high noise is: 1,3 and 6
  ...
  double noiseLow = noiseLivetime[2]+noiseLivetime[4]+noiseLivetime[5];
  double noiseHigh = noiseLivetime[1]+noiseLivetime[3]+noiseLivetime[6];
